...
sets, fixed percentages and conclusion-string checks are forbidden. Artifact
...
- TC-11 的两份成果共享服务端 `business_gate_outcome`：18 项逐功能台账、四条正式上线 Gate 和单列辅助指标。`verifier_status=passed` 只证明来源、公式和文件结构，不能盖过 `business_gate_outcome.status=failed` 的“不得上线”；任一来源合同或 Verifier 失败都必须保持 Artifact/EffectReceipt 红灯。
