...
Demo 2 当前 Runtime 事实（2026-08-21，受控内部执行修订）：route mutation 只记录本轮方式并保持 `not_started`；固定客户 A 选择 Adaptive Swarm 后可用独立 execution command 启动。服务端在单 API 进程 memory 内创建 `Demo2ExecutionSnapshot`，三个初始 Worker 调用 `deepseek-v4-pro`，文件收入冲突增派第四个核验 Worker，生成 5 个 `SharedArtifactVersion`，sequence 15 完成且 `ExecutionReceipt.external_side_effect=none`。第一轮 live 墙钟 8799 ms、4/4 模型输出采用，四次调用 4956/4268/3590/3665 ms；第二轮浏览器 manifest 记录 seq 9/10/15、4 workers/5 artifacts 与 6 张截图。封口为 Python `178 passed, 1 skipped in 6.31s`、浏览器 `41 passed (2.0m)`，Ruff、lint、build、governance 和 diff-check 通过；长期服务 3000/8010 仍运行，health 为 `deepseek-v4-pro`、checkpoint/task_store memory。该结论只在固定客户 A、项目仿真文件、单进程 memory、无外部动作范围内为 `Limited Verified`。`X-User-Id` 仍是 P0 占位，API 重启不恢复 Demo 2 执行，无后台队列、真实 Connector、生产身份、竞品实测或用户研究。
