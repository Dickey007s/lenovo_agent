...
function phaseFromStatus(status: string | undefined): HarnessPhase {
  if (status === "ready_to_execute") return "ready_to_execute";
...
export function HarnessActivityPane({ state }: { state: HarnessActivityState | null }) {
  ...
  return <section className="harness-activity-pane" aria-labelledby="harness-activity-title">
    <header><div className="harness-agent-avatar"><IconSparkles aria-hidden="true" /></div><div><span>Agent 此刻在做什么</span><h2 id="harness-activity-title">{state.scenarioTitle}</h2></div><b className={`is-${state.connection}`}><i />{state.connection === "live" ? "实时" : state.connection === "reconnecting" ? "重连中" : state.connection === "offline" ? "离线" : "连接中"}</b></header>
...
export function HarnessWorkbench({ initialDemo = "demo1", onActivityChange }: { initialDemo?: HarnessDemo; onActivityChange?: (state: HarnessActivityState | null) => void }) {
  ...
  useEffect(() => {
    ...
    setRun(null); setActivity([]); setError(""); setStartAttempted(false); setSelectedFile(null);
    const preview = scenarios.find((item) => item.demo === selectedDemo) ?? null;
    setScenario(preview);
    if (!preview) return;
    void fetch(`${API_BASE}/v1/harness/scenarios/${encodeURIComponent(preview.scenario_id)}`, { headers: HEADERS })
      .then((response) => response.ok ? response.json() as Promise<unknown> : null)
      .then((body) => { if (generation !== generationRef.current) return; const detail = body ? normalizeScenario(body) : null; if (detail) setScenario(detail); })
      .catch(() => undefined);
