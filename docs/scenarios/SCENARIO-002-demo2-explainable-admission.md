...
3. **解释**：用户展开某项任务，查看路由依据和允许模式，而不是查看模型思维链或内部评分过程。
4. **本次选择**：客户 A 用户选择某个允许模式，选择范围限定为“仅本次运行”；本纵切不实现拖拽调序或长期排序偏好。
...
当前已有协议、API、完整浏览器工程代理、完整 Python 回归与截图证据，但没有目标用户研究、真实 Connector、动态 Worker、真实计费、成本账单或端到端时延基准。因此本场景只在单进程固定演示范围内标为 `Verified`，不能升级为用户价值或 Adaptive Swarm 已验证。
