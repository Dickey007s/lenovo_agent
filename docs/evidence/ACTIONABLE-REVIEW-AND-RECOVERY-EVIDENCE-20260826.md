...
- 同一恢复状态若耗尽预算并进入 `stopped/bounded`，前台不再显示不可执行的 resume 暗示。
  它公开旧 Run 已终止、保留项和候选 Branch；用户补充方向后以一条 Branch 创建新 Run，
  浏览器不会向 terminal Run 发送 control。
...
| Playwright | `20 passed` | 处置单、文件高亮、接受/暂缓回执、反馈、新 Run、候选消歧、只恢复目标 Branch、重连恢复、历史失败恢复与预算终态续办 | 用户理解和业务价值 |
...
| `dr-0030-bounded-branch-recovery.png` | 已捕获 | 预算终态显示保留项、未完成 Branch、补充方向和新 Run 入口；确定性测试未发送旧 Run control | 用户理解提升或真实 Provider 的下一轮质量 |
...
| live Run `harness:1e1f45d47a4c4099912b7369b19f2f0d` | 1 轮、3 次模型调用、两次结构拒绝、5 条 stopped Branch、draft Artifact v1 | 真实 Provider 形成 `analysis_output + budget_exhausted` 终态并暴露旧“继续”文案不成立 | 新界面有效、模型质量或 Durable State |

脱敏运行事实见
[`dr-0030-actionable-review-live-run.json`](manifests/dr-0030-actionable-review-live-run.json)。
预算终态的脱敏事实见
[`dr-0030-bounded-terminal-recovery-live-run.json`](manifests/dr-0030-bounded-terminal-recovery-live-run.json)。
...
`dr-0030-bounded-branch-recovery.png` 为
`B477D3D99A1A2E79EAE6623CDEF92F0C27488E442F87A82622F54B8DF0B0E058`。
...
- 预算终态的 Branch 只用来构造新的任务目标；新 Run 仍重新检索整个资料库，不保证沿用
  旧 Run 的文件选择，也不是旧模型调用或检查点续跑。
