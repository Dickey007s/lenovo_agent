...
> 生命周期说明：本文件仍是 DR-0016 的限定基础证据。DR-0017 后续将 FORTE 设为唯一产品入口；DR-0018 再扩展为七路径、两次模型调用和只读结果。原运行、测试数字与 hash 保持历史事实；这里的六路径、`ready_to_execute` 与 6 张截图不是当前 UI/API/终态证据。
