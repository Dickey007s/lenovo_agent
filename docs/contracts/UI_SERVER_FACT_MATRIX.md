...
| Business Gate decision | deterministic files may be valid while source-derived business conditions fail | Artifact and EffectReceipt `business_gate_outcome.status/decision/gates[]/auxiliary_metrics[]/records[]` | show the non-green decision and formal formulas before auxiliary metrics and the 18-row ledger; keep Artifact verification separate | business failure means file generation failed, a green Artifact means release approved, or the browser computed the decision |
