# UI—服务端事实矩阵（Demo 1 与报价工作区）

> 状态：协议映射 `Ready`，交互决策 `DR-0005` 为 `Draft`，报价核算决策 `DR-0006` 在固定演示报价、当前公式、revision 协议和被测恢复路径内为 `Verified`。PR 4-6 的 TaskSnapshot、工件、控制、恢复和历史版本映射仍有工程证据；收到用途不清反馈后，前台已按业务任务、单次开始、决定后果和完成成果重排。后续反馈又要求把内部来源改为明确的演示数据标签、把终态动作改为创建并启动独立新 Task，并将决策控制限制在 Tasks 工作区。报价反馈则要求当前屏幕、保存版本和 Agent 回答共享确定性核算事实。自动化只能验证投影与调用语义，不能代替真实用户理解、真实 Connector、生产报价规则或多实例一致性验证。
...
| Action Gate Tray | 某个副作用动作是否可执行 | `RunSnapshot.risk/control_plan/permit/tool_result/thread_id`；注册 capability 的 Action 由当前 WorkspaceArtifact 内容与 revision 确定性重建 | Conversation `action.proposed`、Run API/SSE、绑定 Thread 的 continue stream | 补证据、审批、Authorize；结果未确认送达时重新读取 | 模型参数/source_refs 不直接执行，内容不匹配或 Artifact 竞态时不创建动作；未解析纯文本收件人/附件类别固定 deny 且自报 evidence 不能解锁；跨 Thread continue 拒绝；完成说明单进程重放同一消息 | Permit token、策略内部秘密、模型伪造 payload/source、内部重放缓存 |
...
报价前端即时投影只用于让用户核对当前未保存输入，不是独立业务真值。跨 API 边界的报价身份、标准价、底线、规范化总计和 `needs_review` 仍由 Workspace 服务端事实约束；Agent 回答必须重新从这些事实计算。基线演示值与来源记录见 [`DR-0006`](../decisions/DR-0006-deterministic-quote-calculation.md) 与对应 Evidence：全量 Python `105 passed, 1 skipped (2.63s)`，报价/Conversation 聚焦 `51 passed (1.55s)`，报价浏览器 `14 passed (23.5s)`，完整浏览器 `26 passed (59.6s)`，Ruff、lint、build 通过。`Verified` 只覆盖上述固定范围；Workspace 锁/revision 仍是单 API 进程内机制，没有数据库 CAS 或多实例验证。
...
| DR-0006 报价修订（限定范围 Verified） | 当前行即时核算、指标区分、错误停算、待复核、版本/晚到结果冲突恢复与可追溯 Agent 回答 | Decimal/BigInt 同公式；Workspace Artifact/revision/request epoch；服务端来源与字段所有权；Artifact/Thread 绑定 Action；确定性 unresolved-context deny；结果完成消息重放 | [`QUOTE-WORKSPACE-DETERMINISTIC-CALCULATION-EVIDENCE-20260811`](../evidence/QUOTE-WORKSPACE-DETERMINISTIC-CALCULATION-EVIDENCE-20260811.md)：提交 `2f9866f + fe865bd`；Python `105 passed, 1 skipped (2.63s)`；聚焦 `51 passed (1.55s)`；报价浏览器 `14 passed (23.5s)`；完整浏览器 `26 passed (59.6s)`；不证明用户理解、真实 Connector/计价或多实例 CAS |
