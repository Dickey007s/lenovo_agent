...
| Record a Finding choice | user accepts, declines, defers or cancels one Finding and may add feedback | versioned/idempotent `command=decision`; `decision_requests[]`; `decision_records[]`; `decision_recorded` | close/Escape records `defer`; defer remains actionable; cancel closes the packet without pretending the evidence was rejected; accept binds one owned option | file edit, external action, approval correctness or automatic follow-up |
