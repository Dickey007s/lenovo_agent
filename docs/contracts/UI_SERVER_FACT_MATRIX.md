...
> 状态：协议映射 `Ready`，交互决策 `DR-0005` 为 `Draft`，报价核算决策 `DR-0006` 在固定演示报价、当前公式、revision 协议和被测恢复路径内为 `Verified`；`DR-0007` 在固定客户回复草稿到治理 Run 的单一纵切内为 `Verified`。PR 4-6 的 TaskSnapshot、工件、控制、恢复和历史版本映射仍有工程证据；收到用途不清反馈后，前台已按业务任务、单次开始、决定后果和完成成果重排。后续反馈又要求把内部来源改为明确的演示数据标签、把终态动作改为创建并启动独立新 Task，并将决策控制限制在 Tasks 工作区。报价反馈则要求当前屏幕、保存版本和 Agent 回答共享确定性核算事实。自动化只能验证投影与调用语义，不能代替真实用户理解、真实 Connector、生产报价规则或多实例一致性验证。
...
| DR-0007 Task 工件动作桥（限定范围 Verified） | 完成态列出成果，当前已验证客户回复提供“准备发送”；Gate 显示绑定版本、演示目标、L4 风险、确认原因和拒绝后果 | TaskService 校验 Commit/ArtifactVersion/Verification；RunService 持久化并重校验 `TaskArtifactBinding`；Policy/Evidence/Approval/Permit/Gateway 延续 Demo 3 事实链 | [`TASK-ARTIFACT-ACTION-BRIDGE-20260813`](../evidence/DEMO1-DEMO3-TASK-ARTIFACT-ACTION-BRIDGE-EVIDENCE-20260813.md)：实现提交 `4f500c6`；Python `112 passed, 1 skipped`；完整浏览器 `29 passed`；Ruff、lint、build 与治理测试通过；不证明真实发送、通用工件动作、跨重启 Run 幂等或用户价值 |
