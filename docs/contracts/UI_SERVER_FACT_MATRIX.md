...
| Evidence gap | the Agent has not produced an adoptable result for one or more bounded Branches | branch `status=waiting_input`, `evidence_gaps[].branch_id`, `evidence_gate`, `next_step.candidate_branch_ids/recovery_kind` | prior rounds/branches/versions remain visible; no new call starts until a Branch resume | claim that a candidate file is wrong or guarantees truth |
| Agent gap recovery sheet | user sees whether structure, source location or evidence coverage failed, what was attempted/preserved and the smallest next action | latest Round `next_step.recovery_kind`, bound Branch objective/status/input/verified refs, Gap candidates and Planner/Analyst `called/output_used` | opening has no mutation; guidance is optional; waiting Run may steer then resume only that Branch; terminal Run creates a new task | raw validator text, invented row/highlight, mandatory source edit or replay of a terminal provider call |
| Gap/Branch review page | user can inspect where the gap occurred, what it says and which candidate/missing files are available | `round_number`, business Branch title, `evidence_gaps[]`, Branch `missing_file_refs`, Preview GET | open/close has no server mutation unless a pending structured decision is explicitly deferred; resume remains a separate versioned command | raw Branch ID, claim that candidate files solve the gap, invented diff |
...
| Evidence location state | user sees whether a quote has one, many or no safe-preview matches | `next_step.evidence_resolutions[]`: current `exact/ambiguous/unavailable`; DR-0032 adds `stale/rejected` with source revision | select a real candidate or preserve/decline the unresolved item | semantic entailment, invented coordinates, current Runtime does not yet emit `stale/rejected` |
