...
10. 文档列表与逐规则事实区按内容容器宽度自适应，不再依据整个 viewport 强制两列。窄中央工作区退成一列；只有卡片和事实单元达到可读宽度时才增加列数。这个浏览器布局事实不改变任何法务结论或服务端计数。
...
- 1440 px 普通三栏和 390 px 页面均检查文档卡宽度、文件名行数、展开事实宽度与页面溢出；截图只证明被测布局，不证明用户理解。
