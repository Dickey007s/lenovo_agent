...
3. 每项风险从 PRD 规则、功能优先级、测试原因类型和异常环境数量推导，并只保留最高等级。原因类型的基础等级来自 PRD 等级单元格，未知或歧义等级 fail closed。固定样本的严重 4、主要 2、次要 2 和风险总数 8 是期望结果，不是实现或 Verifier 常量。
...
6. 输出真实 `上线合规与风险报告.docx` 和 `上线功能风险逐项台账.csv`。DOCX 必须含结构化 Gate、18 项矩阵、风险、未提测和整改计划表；CSV 一行一个 PRD 功能并可独立复算。风险总数、未提测总数、标题、摘要和复核引导全部由当前台账动态生成。
...
- 引擎级变异：F02 修复后完整 Effect 与两份 Artifact 仍通过，风险总数从 8 动态变为 7，DOCX/CSV/前台摘要无陈旧数量；PRD 把“功能缺陷”等级改为次要时 F05 随来源变化，未知或歧义等级拒绝。
