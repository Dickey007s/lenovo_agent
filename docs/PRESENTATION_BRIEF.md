...
| 11 | Demo 2 回到“输入、过程、输出”：用户用普通指令要求核对产品上线、搜索 Agent 运行和用户交互三条工作线；系统先解释为什么采用单 Controller、固定流程或受限 Worker，再由用户决定是否启动高成本路线 | 十份公开输入按三条工作线分组；3 个首波业务工作包、2 个依赖工作包、实际执行/候选采用回执、原文定位、局部失败影响和逻辑 ArtifactVersion v1/v2；明确当前不是 DOCX/CSV 下载物 | `DR-0053/55`、`SCENARIO-039/041/042`；当前每批最多 3 个进程内只读 Analyst Worker，WorkUnit/Contribution 可在单主机 PostgreSQL 恢复，但不是 durable queue/lease 或分布式 Swarm，也没有质量/成本优势结论 |
