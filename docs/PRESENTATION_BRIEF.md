...
| 11 | Demo 2 先解释为什么采用单 Controller、固定流程或受限 Worker，再由用户决定是否启动高成本路线；每个工作包的实际执行、候选返回和成果采用分别留痕 | 拓扑准入理由、完整 Branch DAG、3 个首波工作包、实际执行回执、候选成果、原文定位、下一波 ready、统一 ArtifactVersion v1/v2 | `DR-0053/55`、`SCENARIO-039/041`；当前每批最多 3 个进程内只读 Analyst Worker，WorkUnit/Contribution 可在单主机 PostgreSQL 恢复，但不是 durable queue/lease 或分布式 Swarm，也没有质量/成本优势结论 |
