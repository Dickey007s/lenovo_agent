...
- 确定性 Gate：唯一 START；拨号前时段 Gate；接通/未接通互斥；每日 3 次/每小时 1 次；录音先于身份；身份先于欠款引导；第三方禁呼；本人态度分支；无效通话回频次判断；高风险转人工；六类终态；专业说明来源完整；`external_action=none`。
- 成果验收：下载的 DOCX 可打开，首部回答文档用途、采用《专业性说明.md》、执行边界和人工复核责任，正文保留完整流程和六类终态。成果卡与任务结语显眼说明只生成流程设计 DOCX，文档动作不是执行回执。
- 真实 Run：`harness:d1bae6a41cc84004b8db2b1c5be8147b` 为 1 轮、2 次 `deepseek-v4-pro` 调用、13/13 检查、DOCX 独立解析 13/13，见 `DR-0039` Evidence。
...
| TC-10 | 根据专业性说明生成信用卡 M1 逾期用户 AI 外呼催收流程图文档。 | DOCX，13 项检查，无真实外呼 |
