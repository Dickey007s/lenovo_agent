...
| `TC-11` | `pm-014` | 上线合规与风险评估 | MD + 3 个 XLSX | Demo 2 多任务 | 来源推导风险、正式 Gate、结构化报告与台账 | `Limited Verified`（DR-0043、Run `harness:d1a3d9fca21d4e2299ac308bbaf73e1e`）：18 行台账、四条正式 Gate、变异/负向门、DOCX/CSV 下载解析与 PostgreSQL 顺序恢复；固定适配器，没有多 Worker 或真实发布 |
...
- 可证伪变异：F17 异常环境从 4 改为 2 应由严重降为次要；F05 原因改为界面缺陷应由主要降为次要；删除 F02 问题后应移出风险。
- 负向门：重复/未知编号、跨表冲突、未知状态、非法数字、通过数越界、重复环境语义、零分母和任一 Verifier 失败都不得显示可靠报告绿灯。
