...
- 期望路径：冻结三个工作簿 -> 只从 2026 生成正数贷方未付与正数借方未收 CSV -> 另用三期借方未收余额做跨期比较 -> 数值验证 -> 生成两个 CSV 与说明。
...
- 验证：未付 31 条记录、未收 2 条记录与逐行金额/排序由 2026 来源复核；三期来源完整和僵尸比较只归跨期说明。Artifact 卡必须显示期间、口径、用途和内容来源；引用只证明来源范围，不证明生产财务定义正确。
...
| TC-05 | 核对三期往来明细，生成未付统计、未收统计，并判断是否存在僵尸账款。 | 2 个 2026 CSV、1 个三期说明，31/2 条明确为记录数，8 项检查 |
