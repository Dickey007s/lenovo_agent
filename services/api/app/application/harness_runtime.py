...
This module deliberately stops at ``ready_to_execute``.  It is not a second
Demo 1/2/3 executor: the harness plans work, validates the plan against the
server-owned workspace catalog, and hands a reviewable execution boundary to
the UI.  Tool execution is intentionally outside this first vertical slice.
"""
...
class OpenAICompatibleHarnessPlanner:
    ...
    async def plan(self, *, scenario: dict[str, Any], files: list[dict[str, Any]]) -> HarnessPlan:
        ...
        system = (
            ...
            "只输出一个符合 JSON Schema 的 JSON 对象。只能引用 files 中出现的 path；tool 必须来自 allowlisted_tools；"
            "输入文件永远只读，禁止把源文件目录当作输出目录，禁止输出源文件路径或任意本地路径。"
...
class HarnessRuntime:
    ...
    ALLOWED_STATUSES = {"queued", "indexing", "planning", "validating", "ready_to_execute", "failed"}
    MAX_UNITS = 12

    def __init__(self, catalog: HarnessScenarioCatalog, planner: HarnessPlanner) -> None:
    ...
    def _public_dict(item: Any) -> dict[str, Any]:
        if isinstance(item, dict):
            ...
            public["files"] = [
                {
                    key: file[key]
                    for key in ("display_label", "display_group", "display_summary")
                    if key in file
                }
                for file in item.get("files", [])
                if isinstance(file, dict) and "display_label" in file
            ]
            return public
    ...
    async def start(self, owner_id: str, request: HarnessRunStart) -> HarnessRunStartResult:
        ...
        async with self._lock:
            ...
            task = asyncio.create_task(self._run(owner_id, run_id, scenario))
    ...
    def public_snapshot(self, snapshot: HarnessRunSnapshot) -> PublicHarnessRunSnapshot:
        path_to_ref = {
            document.get("path"): f"file-{index:02d}"
            for index, document in enumerate(snapshot.source_documents, start=1)
            ...
            public_plan = PublicHarnessPlan(
                ...
                units=[
                    PublicHarnessPlanUnit(
                        ...
                        input_file_refs=[path_to_ref.get(path, "unmapped-file") for path in unit.input_paths],
    ...
    async def events(self, owner_id: str, run_id: str, after: int = 0):
        ...
        while True:
            async with self._lock:
                ...
                terminal = current.status in {"ready_to_execute", "failed"}
    ...
    async def _run(self, owner_id: str, run_id: str, scenario: dict[str, Any]) -> None:
        try:
            files = self._index_files(scenario)
            await self._set_source_documents(owner_id, run_id, files, str(scenario.get("selection_reason", "按任务说明选择输入文件")))
            await self._transition(owner_id, run_id, "indexing", "workspace_index", "已读取并冻结场景文件索引。", {
                "files": files, "reason": scenario.get("selection_reason", "按任务说明选择输入文件"),
            })
            await self._transition(owner_id, run_id, "planning", "planning_started", "正在根据文件索引生成工作计划。", {})
            started = perf_counter()
            try:
                plan = await self.planner.plan(scenario=scenario, files=files)
    ...
    @staticmethod
    def _index_files(scenario: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def _validate_plan(cls, plan: HarnessPlan, scenario: dict[str, Any], files: list[dict[str, Any]]) -> None:
        ...
        for unit in plan.units:
            unknown_paths = set(unit.input_paths) - allowed_paths
            if unknown_paths:
                raise HarnessPlanError("计划引用了未在文件索引中的路径")
